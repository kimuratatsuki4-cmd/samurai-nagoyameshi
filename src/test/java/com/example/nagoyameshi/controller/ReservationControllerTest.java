...
public class ReservationControllerTest {
    ...
    public void 未ログインの場合は予約せずにログインページにリダイレクトする() throws Exception {
        ...
        mockMvc.perform(post("/restaurants/1/reservations/create").with(Objects.requireNonNull(csrf()))
    ...
    public void 未ログインの場合は予約をキャンセルせずにログインページにリダイレクトする() throws Exception {
        mockMvc.perform(post("/reservations/1/delete").with(Objects.requireNonNull(csrf())))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("http://localhost/login"));
    ...
    public void 有料会員としてログイン済みの場合は他人の予約をキャンセルせずに予約一覧ページにリダイレクトする() throws Exception {
        mockMvc.perform(post("/reservations/21/delete").with(Objects.requireNonNull(csrf())))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/reservations"));
