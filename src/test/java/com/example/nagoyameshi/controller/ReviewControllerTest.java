...
public class ReviewControllerTest {
    ...
    public void 未ログインの場合はレビューを登録せずにログインページにリダイレクトする() throws Exception {
        ...
        mockMvc.perform(
                post("/restaurants/1/reviews/create").with(Objects.requireNonNull(csrf())).param("score", "5")
                        .param("content", "テスト感想"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/login"));
    ...
    public void 未ログインの場合はレビューを更新せずにログインページにリダイレクトする() throws Exception {
        mockMvc.perform(
                post("/restaurants/2/reviews/1/update").with(Objects.requireNonNull(csrf())).param("score", "5")
                        .param("content", "テスト感想"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/login"));
    ...
    public void 有料会員としてログイン済みの場合は他人のレビューを更新せずに店舗詳細ページにリダイレクトする() throws Exception {
        mockMvc.perform(
                post("/restaurants/2/reviews/2/update").with(Objects.requireNonNull(csrf())).param("score", "5")
                        .param("content", "テスト感想"))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/restaurants/2"));
    ...
    public void 未ログインの場合はレビューを削除せずにログインページにリダイレクトする() throws Exception {
        mockMvc.perform(post("/restaurants/2/reviews/1/delete").with(Objects.requireNonNull(csrf())))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/login"));
    ...
    public void 有料会員としてログイン済みの場合は他人のレビューを削除せずに店舗詳細ページにリダイレクトする() throws Exception {
        mockMvc.perform(post("/restaurants/2/reviews/2/delete").with(Objects.requireNonNull(csrf())))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/restaurants/2"));
